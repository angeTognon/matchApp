...
function verify_jwt($token) {
    try {
        error_log("Vérification JWT - Token reçu: " . $token);
        ...
        error_log("Vérification JWT - Headers décodés: " . json_encode($headers));
        ...
        error_log("Vérification JWT - Signature reçue: " . bin2hex($signature));
        error_log("Vérification JWT - Signature attendue: " . bin2hex($expected_signature));
        error_log("Vérification JWT - Signatures égales: " . (hash_equals($signature, $expected_signature) ? 'oui' : 'non'));
